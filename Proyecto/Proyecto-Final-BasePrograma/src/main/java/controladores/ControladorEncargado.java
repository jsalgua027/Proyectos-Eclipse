...
public class ControladorEncargado {
	...
	public void borrarCliente(Encargado e) {
		...
		Encargado aux = null;
		this.em.getTransaction().begin();// se inicia la transacci�n con la base de datos para el borrado
	...
	public Encargado findByPK(int codEncar) {
		...
		this.consulta = em.createNativeQuery("Select * from encargado where codEncar = ?", Cliente.class);
	...
	public Encargado findByNif(String nombre) {
		this.em = entityManagerFactory.createEntityManager();
		Encargado aux = null;
		this.consulta = em.createNativeQuery("Select * from encargado where nomEncar = ?", Cliente.class);
